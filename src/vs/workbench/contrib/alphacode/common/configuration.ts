	...
	properties: {
		"alphacode.ai.provider": {
			...
			description: localize(
				"alphacode.ai.provider",
				"AI provider to use for code assistance",
			),
		...
		"alphacode.ai.apiKey": {
			...
			description: localize(
				"alphacode.ai.apiKey",
				"API key for the selected AI provider (stored securely)",
			),
			scope: ConfigurationScope.APPLICATION,
		},
		"alphacode.ai.endpoint": {
			...
			description: localize(
				"alphacode.ai.endpoint",
				"Custom endpoint URL for the AI provider (optional)",
			),
			scope: ConfigurationScope.APPLICATION,
		},
		"alphacode.ai.model": {
		...
		"alphacode.ai.temperature": {
			...
			description: localize(
				"alphacode.ai.temperature",
				"Temperature for AI responses (0-2, higher = more creative)",
			),
			scope: ConfigurationScope.APPLICATION,
		},
		"alphacode.context.indexWorkspace": {
			...
			description: localize(
				"alphacode.context.indexWorkspace",
				"Enable workspace indexing for better context awareness",
			),
		...
		"alphacode.context.maxFiles": {
			...
			description: localize(
				"alphacode.context.maxFiles",
				"Maximum number of files to index (set to 0 for no limit)",
			),
			scope: ConfigurationScope.RESOURCE,
		},
		...
		"alphacode.chat.saveSessions": {
			...
			description: localize(
				"alphacode.chat.saveSessions",
				"Save chat sessions across workspace sessions",
			),
			scope: ConfigurationScope.RESOURCE,
		},
