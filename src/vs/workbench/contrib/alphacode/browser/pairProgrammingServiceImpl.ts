...
export class AlphaCodePairProgrammingService extends Disposable implements IAlphaCodePairProgrammingService {
	declare readonly _serviceBrand: undefined;

	private readonly _onDidGenerateSuggestion = this._register(new Emitter<IPairProgrammingSuggestion>());
	readonly onDidGenerateSuggestion: Event<IPairProgrammingSuggestion> = this._onDidGenerateSuggestion.event;

	private readonly _onDidChangeCursorContext = this._register(new Emitter<ICursorContext>());
	readonly onDidChangeCursorContext: Event<ICursorContext> = this._onDidChangeCursorContext.event;

	private config: IPairProgrammingConfig;
	private pendingSuggestions: Map<string, IPairProgrammingSuggestion> = new Map();
	...
	private loadConfig(): IPairProgrammingConfig {
		const stored = this.storageService.get(STORAGE_KEY_CONFIG, StorageScope.WORKSPACE);
		if (stored) {
			try {
	...
	async requestSuggestion(context: ICursorContext): Promise<IPairProgrammingSuggestion | undefined> {
			...
			const timeout = setTimeout(async () => {
				...
				try {
					...
					if (suggestion && suggestion.confidence >= this.config.minConfidenceThreshold) {
						...
						if (this.config.mode === PairProgrammingMode.Active && suggestion.confidence > 0.9) {
	...
	private async generateSuggestion(context: ICursorContext): Promise<IPairProgrammingSuggestion | undefined> {
...
Provide a helpful suggestion for this context.`
		...
		const response = await this.aiService.sendMessage(messages, { maxTokens: 500 });
