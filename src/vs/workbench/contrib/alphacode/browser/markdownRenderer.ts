...
export class MarkdownRenderer {
	private readonly trustedTypesPolicy = createTrustedTypesPolicy('alphaCodeMarkdownRenderer', { createHTML: value => value });
	...
	private renderParagraph(text: string): HTMLElement {
		...
		if (this.trustedTypesPolicy) {
			temp.innerHTML = this.trustedTypesPolicy.createHTML(html) as unknown as string;
		} else {
			temp.innerHTML = html;
		}
	...
	private renderCodeBlock(lines: string[], startIndex: number): { element: HTMLElement; nextIndex: number } {
		...
		append(header, $('span.alphacode-code-block-language', undefined, language));

		const copyButton = append(header, $('button.alphacode-code-block-copy', undefined, 'Copy')) as HTMLButtonElement;
		copyButton.onclick = () => {
	...
	private renderList(lines: string[], startIndex: number): { element: HTMLElement; nextIndex: number } {
	...
	private renderInline(text: string): string {
		...
		// Code (backticks)
		result = result.replace(/`([^`]+)`/g, '<code class="alphacode-inline-code">$1</code>');
		...
		// Links
		result = result.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" class="alphacode-link">$1</a>');
	 ...
	 */
	public extractCodeBlocks(markdown: string): Array<{ language: string; code: string }> {
