...
const DEFAULT_PATTERNS: ISecretPattern[] = [
	...
	{
		name: 'AWS Secret Key',
		pattern: /aws_secret_access_key\s*=\s*[\'"]([A-Za-z0-9/+=]{40})[\'"]|[\'"]([A-Za-z0-9/+=]{40})[\'"](?=.*aws)/gi,
	...
	},
	{
		...
		pattern: /['"]?[Aa]pi[_-]?[Kk]ey['"]?\s*[:=]\s*['"]([A-Za-z0-9-_]{20,})['"]|['"]?[Aa]uth[_-]?[Tt]oken['"]?\s*[:=]\s*['"]([A-Za-z0-9-_]{20,})['"]/g,
...
export class AlphaCodeSecurityService extends Disposable implements IAlphaCodeSecurityService {
	...
	private loadConfig(): ISecurityConfig {
		const stored = this.storageService.get(STORAGE_KEY_CONFIG, StorageScope.APPLICATION);
		if (stored) {
			try {
	...
	private saveConfig(): void {
		...
		this.storageService.store(STORAGE_KEY_CONFIG, JSON.stringify(serializable), StorageScope.APPLICATION, StorageTarget.MACHINE);
	}
